...
let CalendarBase = class CalendarBase extends Component {
    ...
    createContentHeader() {
        ...
        const shortNames = this.getCultureValues().length > 0 && this.getCultureValues() ? this.shiftArray(((this.getCultureValues().length > 0 && this.getCultureValues())), this.firstDayOfWeek) : null;
...
let DateRangePicker = class DateRangePicker extends CalendarBase {
    ...
    resetHandler(e) {
        ...
        this.hide(e);
        if (closest(this.element, 'form')) {
            let element = this.firstHiddenChild;
            let keyupEvent = document.createEvent('KeyboardEvent');
            keyupEvent.initEvent('keyup', false, true);
            element.dispatchEvent(keyupEvent);
        }
