...
(function (Input) {
    ...
    function calculateWidth(element, container) {
        let elementWidth = element.clientWidth - parseInt(getComputedStyle(element, null).getPropertyValue('padding-left'));
        ...
        if (this.enableHtmlSanitizer) {
            for (let i = 0; i < fileData.length; i++) {
                ...
                if (sanitizeFileName != fileData[i].name) {
                    let encodedFileName = targetFiles[i].name.replace(/[\u00A0-\u9999<>\&]/g, function (i) {
...
class SignatureBase extends Component {
    ...
    // eslint-disable-next-line
    mouseDownHandler(e, canvas, panStart, zoomState) {
...
let Rating = class Rating extends Component {
    ...
    renderItemContent(spanEle, val, index, isrerender = false) {
        if (isrerender) {
    ...
    triggerChange(e, val, isInteracted = true) {
    ...
    handleNavigation(e, isIncrease) {
        if ((!isIncrease && (this.value > this.min)) || (isIncrease && (this.value < this.itemsCount))) {
            let currentValue = (this.precision === PrecisionType.Full) ? 1 : (this.precision === PrecisionType.Half) ? 0.5 :
